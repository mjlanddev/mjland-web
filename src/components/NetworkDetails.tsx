  ...
  useEffect(() => {
    const fetchNetworkData = async () => {
      ...
      try {
        ...
        const networkData = await tmdbService.getNetworkDetails(networkId);
        setNetwork(networkData);

        const sectionData = await Promise.all(
          genres.map(async (genre) => {
            const movies = await tmdbService.getMoviesByNetwork(networkId, genre.id);
            const tvShows = await tmdbService.getTVByNetwork(networkId, genre.id);
    ...
    <div className="min-h-screen bg-bg text-white">
      <div className="relative h-[50vh] flex flex-col items-center justify-center pt-20 overflow-hidden">
        ...
        >
          {network.logo_path ? (
            <LazyImage
              src={getImageUrl(network.logo_path, 'original')}
              alt={network.name}
              className="h-32 md:h-48 object-contain mb-8 brightness-0 invert drop-shadow-[0_0_30px_rgba(255,255,255,0.2)]"
              referrerPolicy="no-referrer"
            />
          ) : (
            <h1 className="text-6xl font-bold mb-8 tracking-tight">{network.name}</h1>
          )}
        </motion.div>
      </div>
          ...
          <MovieRow
            ...
            fetchNextPage={async (page) => {
              const [m, t] = await Promise.all([
                tmdbService.getMoviesByNetwork(network.id, section.id, page),
                tmdbService.getTVByNetwork(network.id, section.id, page)
              ]);
