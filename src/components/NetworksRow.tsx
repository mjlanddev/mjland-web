...
const NETWORK_IDS = TMDB_CONFIG.networks.map(n => n.id);

export const NetworksRow = () => {
    ...
    const fetchNetworks = async () => {
      try {
        const results = await Promise.all(
          NETWORK_IDS.map(id => tmdbService.getNetworkDetails(id))
        );
        setNetworks(results);
      } catch (error) {
      }
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-3 px-4 md:px-6">
        <h2 className="text-xl md:text-2xl font-bold text-white tracking-tight">Streaming Services</h2>
      </div>
